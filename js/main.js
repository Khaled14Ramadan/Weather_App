...
let nextDay = document.getElementsByClassName("nextDay"),
    nextDayDate = document.getElementsByClassName("nextDay-date"),
...
async function getWeatherData(currentCity='cairo'){
    apiResponse = await fetch(`https://api.weatherapi.com/v1/forecast.json?key=d38a6ead8c4847a69da151854220607&q=${currentCity}&days=3`)
...
function displayTodayWeather(){
   ...
   function displayNextDayWeather(){
     for( let i=0; i<nextDay.length;i++){
        let d = new Date(responseData.forecast.forecastday[i+1].date);
        console.log(d.getDate());
      nextDay[i].innerHTML= days[d.getDay()];
      nextDayDate[i].innerHTML =`${d.getDate()} ${ monthName[d.getMonth()]}`
